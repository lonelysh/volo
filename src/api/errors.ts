...
export function classifyError(status: number, body: ApiErrorBody | undefined, raw?: unknown): MiniMaxError {
  // 1. HTTP 状态码优先
  if (status === 401 || status === 403) {
    const msg = body?.error?.message ?? body?.message ?? body?.base_resp?.status_msg ?? "鉴权失败";
    return new MiniMaxError({ kind: "auth", message: msg, status, raw });
  }
  if (status === 404) {
 ...
 */
export function networkError(err: unknown): MiniMaxError {
  if (err instanceof MiniMaxError) return err;
  const name = (err as { name?: string })?.name;
  const message = (err as { message?: string })?.message ?? "网络错误";
  if (name === "AbortError") {
    return new MiniMaxError({ kind: "network", message: "请求已取消" });
  }
  return new MiniMaxError({ kind: "network", message });
}
