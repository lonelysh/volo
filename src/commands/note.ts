...
async function runNoteCommand(plugin: MiniMaxPlugin, id: NoteCommandId, file: TFile, source: string) {
...
async function insertResult(plugin: MiniMaxPlugin, file: TFile, content: string, id: NoteCommandId) {
  ...
  if (view) {
    ...
    // 没有激活编辑器就追加到尾部
    const sep = original.endsWith("\n") ? "\n" : "\n\n";
    await plugin.app.vault.modify(file, original + sep + "## MiniMax 输出\n\n" + content + "\n");
  }
}